...
def main():
    ...
    for i, checkpoint in enumerate(checkpoints):
        if high_level_args.eval_mode in ['all', 'completion']:
            ...
            task, model, generator, itr, step = evaluate_utils.load(args, re_init_weight=high_level_args.re_init_weight)
            ...
            completions, gt_completions, gen_metrics, actual_metrics = evaluate_utils.generate_completions(
            ...
            gt_completion_output_filename = os.path.join(high_level_args.save_path,
                                                      'GT_completions_{}.txt'.format(filename_suffix))
        ...
        if high_level_args.eval_mode in ['all', 'singletoken']:
            ...
            pdb.set_trace()
            for i in single_predicted_tokens:
                while 4 in i:
                    i.remove(4)
                while 5 in i:
                    i.remove(5)
